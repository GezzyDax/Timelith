...
func SetupRouter(cfg *config.Config, db *database.DB, settingsService *settings.Service, setupRequired bool) *fiber.App {
	...
	app.Use(func(c *fiber.Ctx) error {
		...
		if !setupRequired && cfg.JWTSecret != "" {
	...
	api.Use(SetupMiddleware(setupRequired))
	...
	// Auth routes (only if setup is complete)
	if !setupRequired {
		auth := api.Group("/auth")
